...
export default function RogHero({ item }: { item: GearItem }) {
  ...
  return (
    <section className="relative min-h-screen bg-[#050505] text-white font-sans overflow-hidden">
      {/* Hex grid background */}
      ...
      {/* Gradient orbs */}
      <div className="absolute top-1/4 -left-32 w-96 h-96 bg-[#ff1a1a]/[0.04] rounded-full blur-[120px]" />
      <div className="absolute bottom-1/4 -right-32 w-80 h-80 bg-[#ff1a1a]/[0.03] rounded-full blur-[100px]" />

      {/* Top accent line */}
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#ff1a1a] to-transparent opacity-60" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 min-h-screen flex flex-col justify-center">
        ...
        <motion.div
          ...
          className="max-w-3xl"
        >
          <span className="text-[10px] font-black tracking-[0.4em] text-[#ff1a1a] uppercase mb-3 block">
          ...
          <div className="h-6 md:h-7 flex items-center">
            <span className="text-xs md:text-sm font-mono text-zinc-500 tracking-wider">
              <span className="text-[#ff1a1a]">&gt;</span> {typed}
              <span className="inline-block w-[2px] h-4 bg-[#ff1a1a] ml-1 animate-pulse" />
            </span>
          </div>
        </motion.div>

        {/* Main showcase: laptop + spec cards */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12 items-center">
          ...
          >
            <div className="relative w-full max-w-lg">
              {/* Glow behind device */}
              <div className="absolute inset-0 bg-gradient-to-b from-[#ff1a1a]/10 via-transparent to-transparent blur-[60px] rounded-full" />
              <div className="relative rog-float">
                <Image
                  ...
                  width={600}
                  height={400}
                  className="w-full h-auto object-contain drop-shadow-[0_20px_40px_rgba(255,26,26,0.15)]"
                  priority
                />
              </div>
              {/* Corner accents */}
              <div className="absolute -top-2 -left-2 w-8 h-8 border-t-2 border-l-2 border-[#ff1a1a]/40" />
              <div className="absolute -bottom-2 -right-2 w-8 h-8 border-b-2 border-r-2 border-[#ff1a1a]/40" />
            </div>
          </motion.div>
          ...
          >
            {isSystem && cpuInfo && (
              <div className="rog-card p-4 flex items-center gap-4 group hover:border-[#ff1a1a]/30 transition-all duration-300">
            ...
            {/* Non-system: show first 3 specs */}
            {!isSystem && item.specs.slice(0, 3).map((s) => (
              <div key={s.label} className="rog-card p-4 flex items-center gap-4 group hover:border-[#ff1a1a]/30 transition-all duration-300">
        ...
        >
          ...
          <div className="w-[1px] h-8 bg-gradient-to-b from-[#ff1a1a]/50 to-transparent" />
        </motion.div>
      </div>
    </section>
  );
